...
This file merges parameters for:
1. Physical Simulation (SUMO vs. Kinematic Python)
2. Task Generation (Azure Traces)
...
# 'SUMO': Uses TraCI to interface with real-world road networks (requires SUMO installed).
# 'PYTHON_KINEMATIC': Uses simple 2D movement logic (faster, good for rapid testing).
SIMULATION_MODE = 'SUMO'

# --- Caching Mode ---
# If True, uses the LSTM model to pre-fetch content.
# If False, uses standard reactive caching (cache on miss).
...
# --- Visualization ---
# If SIMULATION_MODE is 'SUMO', this controls sumo-gui.
# If 'PYTHON_KINEMATIC', this controls the Pygame visualizer.
VISUALIZATION = True
VISUALIZER_STAYS_OPEN = False  # Keep window open after run (Python mode only)
...
# Paths to the processed Azure dataset
# These files must be generated by running `process_azure_data.py` and `generate_task_data.py`
DATA_DIR = "data"  # Recommended to keep data in a subfolder
...
# ========================================================================================
# E. TRAFFIC & GEOGRAPHY (SUMO & PYTHON)
# ========================================================================================

# --- Scenario Logic ---
# Used by both modes. In SUMO, 'hotspots' equate to specific busy intersections if implemented,
# or simply selecting specific city maps.
TRAFFIC_SCENARIOS = {
...
# --- SUMO Specifics ---
# Available real-world city maps
SUMO_SCENARIO_POOL = [
...
# LSTM Parameters
PREDICTION_SEQUENCE_LENGTH = 20  # Lookback window
CACHE_UPDATE_INTERVAL = 20  # How often (in steps) to refresh cache predictions
CACHE_UPDATE_PROBABILITY = 0.2  # For reactive fallback
...
# Offloading Logic Selector (Simple vs Prioritized)
