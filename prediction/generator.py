...
class AzureTraceModel:
    def __init__(self):
        # Fallback to random if data is missing (prevents crash on fresh install)
        if not os.path.exists(AZURE_MATRIX_FILE) or not os.path.exists(AZURE_META_FILE):
            print(f"⚠️ WARNING: Azure data files not found in {AZURE_MATRIX_FILE}. Using Random Generation.")
